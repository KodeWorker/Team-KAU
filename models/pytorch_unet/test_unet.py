...
def validate():
    ...
    unet = UNet(in_channels=3, out_channels=BrainSegmentationDataset.out_channels)
    unet.to(device)
    
    state_dict = torch.load(os.path.join(weights, "kaggle_trained_unet.pt"))
    ...
    for p in volumes:
        ...
        for s in range(x.shape[0]):
            ...
            filepath = os.path.join("./img/", os.path.basename(filename))
